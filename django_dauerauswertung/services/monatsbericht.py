...
if True:
    ...
    q_night = f"""
    SELECT * FROM (SELECT time::date AS time, date_part( 'hour', time), max(pegel) AS pegel FROM tsdb_lrpegel lr WHERE time >= '{after_time}' AND time <= '{before_time}' AND time::time <= '06:00' GROUP BY time::date, date_part('hour', time)) T1
