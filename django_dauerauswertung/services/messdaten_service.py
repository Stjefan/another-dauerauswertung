...
if False:
    class MessdatenServiceV3:
        ...
        def get_resu_all_mps(my_mps_data: list[Messpunkt], from_date, to_date):
            ...
            return RandomMessdatenService.get_resudaten(ids_only, from_date.year, from_date.month, from_date.day, from_date.hour*3600, (to_date.hour)*3600+to_date.minute*60+to_date.second)


        def get_terz_all_mps(my_mps_data: list[Messpunkt], from_date, to_date):
            ids_only = [mp.Id for mp in my_mps_data]
            return RandomMessdatenService.get_terzdaten(ids_only, from_date.year, from_date.month, from_date.day, from_date.hour*3600, (to_date.hour)*3600+to_date.minute*60+to_date.second)

        def get_resudaten(ids_mps, jahr, monat, tag, sekunde_from, sekunde_to):
            
            df_mps = []
            for i in ids_mps:
                df_mps.append(RandomMessdatenService.get_resudaten_single(i, jahr, monat, tag, sekunde_from, sekunde_to))
        ...
        def get_terzdaten(ids_mps, jahr, monat, tag, sekunde_from, sekunde_to):
            df_mps = []
            for i in ids_mps:
                df_mps.append(RandomMessdatenService.get_terzdaten_single(i, jahr, monat, tag, sekunde_from, sekunde_to))
            result = pd.concat(df_mps, axis=1, join="inner")
            return result
